...
|---|---|---|---|---|
| **S0** | Fundación de pruebas: Emulator real + scripts CI mínimos | P0 | — | ✅ **COMPLETADO EN MAIN** |
| **S1** | Zero-Trust: Membership admin + Custom Claims + `firestore.rules` (19 colecciones) | P0 | S0 | ✅ **COMPLETADO EN MAIN** (`c2a9b4a`) |
| **S1.5**| Hotfix P0: Inmutabilidad `/users` + `ensureOwnClaims` autoritativo | P0 | S1 | ✅ **COMPLETADO EN MAIN** (`02500f3`) |
| **S2** | RBAC real en cliente (JWT como única fuente) | P0 | S1 | ✅ **COMPLETADO EN MAIN** (`3b0a5b9`) |
| **S3** | Backend único + API Auth + Rate-limit persistente + `storage.rules` | P0 | S1 | ✅ **COMPLETADO EN MAIN** (`a09e722`) |
| **S4** | Autenticación endurecida (sin demo silencioso) | P0 | S2 | ✅ **COMPLETADO EN MAIN** (`6f4afd1`) |
| **S5** | CI/CD bloqueante + alineación de versiones `firebase-admin/functions` | P1 | S0, S3 | ✅ **COMPLETADO EN MAIN** (`ab6dab9`) |
| **S6** | Multi-tenant 13/13 + repositorios + IDs regulatorios server-side | P1 | S1, S3 | ✅ **COMPLETADO EN MAIN** (`ed74a7d`) |
...
| **S11**| Observabilidad y datos sensibles (Sentry/logger) | P2 | S5 | ✅ **COMPLETADO EN MAIN** (`a01df39`) |
| **S12**| Rendimiento (code-splitting) y migración a `exceljs` | P2 | S11 | ✅ **COMPLETADO EN MAIN** (`430376c`) |
| **S13**| **PILOTO PROINTECA END-TO-END (Propanoducto Cardón-Amuay 6")** | P0 comercial | S6, S8, S9, S10 | ✅ **COMPLETADO EN MAIN** (`7bc7090` — **96/100 ENTERPRISE**) |
| **S14**| Tareas Manuales M1–M6 y Auditoría Final de Producción | — | Todos | 🟡 **LISTO PARA EJECUCIÓN MANUAL Y AUDITORÍA** |

---

## 🛠️ 1.5 TAREAS MANUALES DE DESPLIEGUE Y CONFIGURACIÓN (M1 → M6)

> **REGLA DE SEGURIDAD:** GAIS tiene prohibido ejecutar despliegues o manipular credenciales de producción. Estas tareas deben ser ejecutadas por el Administrador en la terminal y consolas.

### 📋 M1: Despliegue Manual de Reglas y Functions (`firebase deploy`)
```bash
npx firebase login
...
### 🔑 M2: Configuración de Secretos en Cloud Functions (Secret Manager)
```bash
npx firebase functions:secrets:set GEMINI_API_KEY
npx firebase functions:secrets:set RESEND_API_KEY
```

### 👑 M3: Bootstrapping del Primer Superadministrador
```bash
node scratch/bootstrapSuperadmin.js tu_correo@empresa.com
```

### 🛡️ M4: Restricción de API Key Web en GCP Console
- Acceder a GCP Console -> APIs & Services -> Credentials -> Browser Key.
- Restringir por HTTP referrers a: `https://industrial-360.vercel.app/*` y `http://localhost:5173/*`.

### 🌐 M5: Dominio Autorizado en Firebase Auth
- Acceder a Firebase Console -> Authentication -> Settings -> Authorized Domains.
- Agregar `industrial-360.vercel.app`.

### 🗄️ M6: Despliegue de Índices de Firestore
```bash
