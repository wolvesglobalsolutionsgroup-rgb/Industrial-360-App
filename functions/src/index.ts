 ...
 */
export const provisionFounderQaAccess = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'El usuario debe estar autenticado para solicitar o gestionar acceso al entorno QA/Preview.'
    );
  }

  const uid = context.auth.uid;
  const email = context.auth.token.email || '';
  const action = (data?.action as string) || 'provision';
  const targetOrgId = (data?.targetOrgId as string)?.trim() || 'prointeca-demo';
  const requestedRole = (data?.role as string)?.trim() || 'gerente';
  const reason = (data?.reason as string)?.trim() || 'Provisionamiento de acceso QA/Preview para evaluación de producto';
  ...
  if (!isQaTenant) {
    throw new functions.https.HttpsError(
      'permission-denied',
      `Acceso denegado: El tenant '${targetOrgId}' no está clasificado como un entorno QA con datos sintéticos.`
    );
  }
  ...
  if (!allowedQaRoles.includes(requestedRole)) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      `Rol no válido para entorno QA: '${requestedRole}'. Roles permitidos: ${allowedQaRoles.join(', ')}`
    );
  }
  ...
  if (action === 'provision') {
    ...
    logger.info(`Acceso QA provisionado exitosamente: uid=${uid}, orgId=${targetOrgId}, role=${requestedRole}`);
    ...
    logger.info(`Acceso QA revocado exitosamente: uid=${uid}, orgId=${targetOrgId}`);
    ...
    throw new functions.https.HttpsError(
      'invalid-argument',
      `Acción no válida: '${action}'. Las acciones soportadas son 'provision' y 'revoke'.`
    );
  }
});
