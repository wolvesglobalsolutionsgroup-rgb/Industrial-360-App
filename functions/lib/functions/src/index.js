...
/**
 * Sprint IC360-S14.2A: Provisionamiento Seguro del Fundador y Acceso QA/Preview
 *
 * Permite que un usuario autenticado provisione o revoque de forma autorizada y auditada
 * una membresía en un tenant QA con datos sintéticos ('prointeca-demo' o 'qa-preview-tenant').
 */
exports.provisionFounderQaAccess = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'El usuario debe estar autenticado para solicitar o gestionar acceso al entorno QA/Preview.');
    }
    const uid = context.auth.uid;
    const email = context.auth.token.email || '';
    const action = data?.action || 'provision';
    const targetOrgId = data?.targetOrgId?.trim() || 'prointeca-demo';
    const requestedRole = data?.role?.trim() || 'gerente';
    const reason = data?.reason?.trim() || 'Provisionamiento de acceso QA/Preview para evaluación de producto';
    // 1. Validar que targetOrgId sea un tenant de QA / datos sintéticos
    const isQaTenant = targetOrgId === 'prointeca-demo' ||
        targetOrgId === 'qa-preview-tenant' ||
        targetOrgId.startsWith('qa-') ||
        targetOrgId.endsWith('-qa') ||
        targetOrgId.endsWith('-demo');
    if (!isQaTenant) {
        throw new functions.https.HttpsError('permission-denied', `Acceso denegado: El tenant '${targetOrgId}' no está clasificado como un entorno QA con datos sintéticos.`);
    }
    // 2. Validar rol solicitado (no permite platformAdmin)
    const allowedQaRoles = ['superadmin', 'gerente', 'supervisor', 'inspector', 'campo'];
    if (!allowedQaRoles.includes(requestedRole)) {
        throw new functions.https.HttpsError('invalid-argument', `Rol no válido para entorno QA: '${requestedRole}'. Roles permitidos: ${allowedQaRoles.join(', ')}`);
    }
    const dbAdmin = (0, firestore_1.getFirestore)();
    const authAdmin = (0, auth_1.getAuth)();
    const membershipRef = dbAdmin.doc(`organizations/${targetOrgId}/memberships/${uid}`);
    const auditLogRef = dbAdmin.collection(`organizations/${targetOrgId}/audit_logs`).doc();
    if (action === 'provision') {
        ...
        await auditLogRef.set({
            ...
            ip: context.rawRequest?.ip || 'N/A',
        });
        logger_1.logger.info(`Acceso QA provisionado exitosamente: uid=${uid}, orgId=${targetOrgId}, role=${requestedRole}`);
        return {
            success: true,
            action: 'provision',
            orgId: targetOrgId,
        ...
        logger_1.logger.info(`Acceso QA revocado exitosamente: uid=${uid}, orgId=${targetOrgId}`);
        return {
            success: true,
            action: 'revoke',
            orgId: targetOrgId,
        ...
        throw new functions.https.HttpsError('invalid-argument', `Acción no válida: '${action}'. Las acciones soportadas son 'provision' y 'revoke'.`);
    }
});
