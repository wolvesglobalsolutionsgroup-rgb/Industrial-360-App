...
exports.verifyDocument = exports.sealDocument = exports.getClientPortal = exports.createClientPortal = exports.issueRegulatoryCode = exports.ensureOwnClaims = exports.setUserCustomClaims = exports.sendEmail = exports.callGeminiProxy = exports.checkRateLimit = exports.rateLimit = exports.requireAuth = void 0;
...
exports.createClientPortal = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'El usuario debe estar autenticado para crear o configurar un portal de cliente.');
    }
    const callerUid = context.auth.uid;
    const callerOrgId = context.auth.token?.orgId;
    const callerRole = context.auth.token?.role;
    ...
    if (callerRole !== 'superadmin' && callerOrgId !== orgId) {
        throw new functions.https.HttpsError('permission-denied', 'No tiene autorización para crear un portal de cliente en esta organización.');
    }
    const dbAdmin = (0, firestore_1.getFirestore)();
...
exports.sealDocument = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Se requiere autenticación para sellar documentos.');
    }
    const callerUid = context.auth.uid;
    const callerOrgId = context.auth.token?.orgId;
    const callerRole = context.auth.token?.role;
    ...
    if (callerRole !== 'superadmin' && callerOrgId !== orgId) {
        throw new functions.https.HttpsError('permission-denied', 'No tiene autorización para sellar documentos en esta organización.');
    }
