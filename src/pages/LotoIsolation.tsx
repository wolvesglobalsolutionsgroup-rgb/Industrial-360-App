...
export default function LotoIsolation() {
  ...
  const handleCreateLotoPoint = async (e: React.FormEvent) => {
    ...
    const newPointObj: Omit<LotoPoint, 'id'> = {
      ...
      energyType,
      isolationMethod,
      lockTagId: lockTagId || `LOCK-${energyType.substring(0, 3).toUpperCase()}-${Math.floor(100 + Math.random() * 900)}`,
      lockColor,
      ptwNumber: ptwNumber || `PTW-2026-${Math.floor(1000 + Math.random() * 9000)}`,
