...
export default function EngineeringTools() {
  ...
  return (
    <div className="space-y-6 pb-12 text-ink">
      {/* PAGE HEADER */}
      <div className="bg-surface rounded-2xl p-6 border border-line shadow-2xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-ink-soft mt-1 max-w-2xl font-medium">
              Calculadoras normativas certificadas (ASME/API/PDVSA), codificador de documentos PIC-01-03-05 y memorias de cálculo en PDF.
            </p>
          </div>

          {/* Quick Nav Tabs */}
          <div className="flex flex-wrap items-center bg-surface-2 p-1.5 rounded-xl border border-line shrink-0 gap-1">
            <button
            ...
            >
              <Disc size={15} className={activeTab === 'mechanical' ? 'text-white' : 'text-brand-500'} />
            ...
            >
              <Calculator size={15} className={activeTab === 'norms' ? 'text-white' : 'text-brand-500'} />
            ...
            >
              <FileCode size={15} className={activeTab === 'pdvsa_coder' ? 'text-white' : 'text-brand-500'} />
            ...
            >
              <Wrench size={15} className={activeTab === 'field_tools' ? 'text-white' : 'text-brand-500'} />
      ...
      {activeTab === 'norms' && (
        <div className="space-y-6">
                ...
                <button
                  ...
                  className={`p-3.5 rounded-2xl text-left border transition-all flex flex-col justify-between ${
                    isSelected
                      ...
                      : 'bg-surface text-ink border-line hover:border-brand-500/50'
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-[10px] font-mono font-extrabold uppercase px-2 py-0.5 rounded-full ${
                      isSelected ? 'bg-white/20 text-white' : 'bg-surface-2 text-ink-soft'
                    }`}>
                  ...
                  <span className={`text-[10px] mt-2 block ${isSelected ? 'text-white/80' : 'text-ink-soft'}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-5 bg-surface rounded-2xl border border-line p-5 shadow-2xs space-y-5">
                ...
                {selectedCalculator.getFields().map(field => (
                  <div key={field.id} className="space-y-1">
                    <div className="flex items-center justify-between text-xs font-bold text-ink">
                      ...
                      {field.unit && (
                        <span className="text-[10px] text-ink-soft font-mono">[{field.unit}]</span>
                      )}
                    </div>
                    ...
                    {field.description && (
                      <p className="text-[10px] text-ink-soft flex items-start gap-1">
                        <Info size={11} className="shrink-0 mt-0.5 text-ink-soft" />
            ...
            <div className="lg:col-span-7 space-y-5">
              {normResults.map((result, idx) => (
                <div
                  key={idx}
                  className="bg-surface rounded-2xl border border-line p-5 shadow-2xs space-y-5"
                >
                  {/* Status Banner */}
                  ...
                  {/* Main Metric Display */}
                  <div className="p-4 bg-surface-2 rounded-xl border border-line font-mono space-y-2">
                    <span className="text-[10px] uppercase font-bold text-ink-soft block tracking-wider">
                      {result.label}
                    </span>
                    ...
                    <div className="flex items-baseline justify-between">
                      ...
                      {result.margin !== undefined && (
                        <div className="text-right">
                          <span className="text-[10px] text-ink-soft block">Margen de Seguridad</span>
                          <span className={`text-sm font-bold ${result.margin >= 0 ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
                        {Object.entries(result.details).map(([key, val]) => (
                          <div
                            key={key}
                            className="p-2.5 bg-surface-2 rounded-xl border border-line flex justify-between items-center"
                          >
                            <span className="text-ink-soft text-[11px]">{key}:</span>
                  ...
                  <div className="p-4 bg-brand-500/10 border border-brand-500/20 rounded-xl space-y-2 text-xs">
                    <span className="font-bold text-brand-500 uppercase flex items-center gap-1.5">
      ...
      {activeTab === 'pdvsa_coder' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 bg-surface p-2 rounded-2xl border border-line shadow-2xs">
            <button
          ...
          {activeAnnexTab === 'coder' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-7 bg-surface rounded-2xl border border-line p-6 shadow-2xs space-y-6">
                <div className="border-b border-line pb-3 flex items-center justify-between">
                  ...
                  {/* Format Toggle */}
                  <div className="flex bg-surface-2 p-1 rounded-xl text-[11px] font-bold border border-line">
                    <button
                      onClick={() => setCodeFormatView('standard')}
                      className={`px-3 py-1 rounded-lg transition-all ${
                        codeFormatView === 'standard' ? 'bg-brand-500 text-white shadow-xs' : 'text-ink-soft'
                      }`}
                    >
                      Estándar (21-Char)
                    </button>
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg transition-all ${
                        codeFormatView === 'short' ? 'bg-brand-500 text-white shadow-xs' : 'text-ink-soft'
                      }`}
                    >
                      Campo (Corto)
                    </button>
                ...
                {/* Live Generated Box */}
                <div className="p-5 bg-surface-2 text-ink rounded-2xl border border-line space-y-3 font-mono">
                  ...
                  <div className="p-3 bg-surface rounded-xl border border-line text-xl sm:text-2xl font-black text-brand-500 tracking-wider text-center break-all">
                    {activeGeneratedCode}
                  </div>
                ...
                <div className="space-y-5 text-xs">
                  ...
                  <div className="p-4 bg-surface-2 rounded-xl border border-line space-y-3">
                    <h4 className="font-bold text-ink uppercase tracking-wider text-[11px] flex items-center justify-between">
                      <span>Grupo 1: Identificación del Proyecto (AABBCCDDEE)</span>
                      <span className="font-mono text-brand-500 font-black">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Filial / Negocio (AA)</label>
                        <select
                          value={pdvsaStdParams.filialAA}
                          onChange={(e) => setPdvsaStdParams(p => ({ ...p, filialAA: e.target.value }))}
                          className="w-full px-2.5 py-1.5 bg-surface border border-line rounded-lg font-bold text-ink"
                        >
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Área Corp / ACE (BB)</label>
                        <select
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Área Geográfica (CC)</label>
                        <select
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Año (DD)</label>
                        <input
                          type="text"
                          maxLength={2}
                          value={pdvsaStdParams.anioDD}
                          onChange={(e) => setPdvsaStdParams(p => ({ ...p, anioDD: e.target.value }))}
                          className="w-full px-2.5 py-1.5 bg-surface border border-line rounded-lg font-bold font-mono text-ink"
                          placeholder="26"
                        />
                      </div>

                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Consecutivo Proy (EE)</label>
                        <input
                          type="text"
                          maxLength={2}
                  ...
                  <div className="p-4 bg-surface-2 rounded-xl border border-line space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Instalación (FF)</label>
                        <select
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Subproyecto (G)</label>
                        <input
                          type="number"
                          min={0}
                          max={9}
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Fase FEL (H)</label>
                        <select
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Actividad (I)</label>
                        <select
                  ...
                  <div className="p-4 bg-surface-2 rounded-xl border border-line space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Disciplina (J)</label>
                        <select
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Tipo (K)</label>
                        <select
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Producto (LL)</label>
                        <select
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Correlativo (MMM)</label>
                        <input
                          type="number"
                          min={1}
                          max={999}
                      ...
                      <div>
                        <label className="block font-bold text-ink-soft mb-1">Revisión (REV)</label>
                        <input
                          type="text"
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                <div className="bg-surface rounded-2xl border border-line p-6 shadow-2xs space-y-4">
                  <h3 className="font-bold text-sm text-ink uppercase flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 bg-surface-2 border border-line rounded-xl text-xs font-mono font-bold outline-none focus:ring-2 focus:ring-brand-500 text-ink"
                    />
                    <Button onClick={handleParseCode} className="bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold">
                      Analizar
                    </Button>
                  </div>

                  {parsedCodeResult && parsedCodeResult.isValid && (
                    <div className="p-4 bg-surface-2 rounded-xl border border-line text-xs font-mono space-y-3">
                      ...
                      <div className="space-y-1.5 text-ink-soft">
                        {Object.entries(parsedCodeResult.details || {}).map(([lbl, val]) => (
                          <div key={lbl} className="flex justify-between items-center text-[11px]">
                            ...
                            <strong className="text-ink">{String(val)}</strong>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="p-5 bg-surface-2 text-ink rounded-2xl border border-line space-y-3 text-xs font-mono">
                  <h4 className="font-bold text-brand-500 uppercase tracking-wider flex items-center gap-1.5">
                    <Info size={14} />
                    <span>Estructura de Codificación PIC-01-03-05</span>
                  </h4>
                  <div className="p-2.5 bg-surface rounded-xl border border-line text-center font-bold text-ink">
                    AABBCCDDEE - FFGHI - JKLLMMM - REV#
                  </div>
                  <ul className="space-y-1 text-ink-soft text-[11px]">
                    <li>• <strong className="text-ink">AABBCCDDEE</strong>: Filial (AA) + ACE (BB) + Area Geo (CC) + Año (DD) + Consecutivo Proy (EE)</li>
                    <li>• <strong className="text-ink">FFGHI</strong>: Instalación (FF) + Subproy (G) + Fase FEL (H) + Actividad (I)</li>
                    <li>• <strong className="text-ink">JKLLMMM</strong>: Disciplina (J) + Tipo (K) + Producto (LL) + Correlativo Doc (MMM)</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          {activeAnnexTab === 'annex_ab' && (
            <div className="bg-surface rounded-2xl border border-line p-6 shadow-2xs space-y-6">
              <div className="border-b border-line pb-3">
                <span className="text-xs font-mono font-bold text-brand-500 uppercase">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                ...
                <div className="p-5 bg-surface-2 rounded-xl border border-line space-y-3">
                  <h3 className="font-bold text-brand-500 uppercase text-sm">
                  ...
                  <div className="space-y-2 text-ink-soft">
                    <div className="p-3 bg-surface rounded-lg border border-line">
                      <strong className="text-brand-500 block mb-1">Rev. A, B, C... Z — Borradores de Trabajo (Emisión Inicial):</strong>
                    ...
                    <div className="p-3 bg-surface rounded-lg border border-line">
                      <strong className="text-emerald-500 dark:text-emerald-400 block mb-1">Rev. 0 — Emisión Original Aprobada:</strong>
                      <p className="text-ink-soft">
                    ...
                    <div className="p-3 bg-surface rounded-lg border border-line">
                      <strong className="text-purple-500 dark:text-purple-400 block mb-1">Rev. 1, 2, 3... n — Revisiones Posteriores Aprobadas:</strong>
                      <p className="text-ink-soft">
            ...
            <div className="bg-surface rounded-2xl border border-line p-6 shadow-2xs space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-xs">
                ...
                <div className="space-y-3">
                  <h3 className="font-bold text-brand-500 uppercase text-xs border-b border-line pb-2">
                    Filial / Negocio (AA) — {PDVSA_FILIALES_C.length} Registros
                  </h3>
                  <div className="max-h-96 overflow-y-auto space-y-1.5 font-mono pr-1">
                    {PDVSA_FILIALES_C.map(f => (
                      <div key={f.code} className="p-2 bg-surface-2 rounded-lg flex justify-between border border-line">
                        <span className="font-bold text-brand-500">{f.code}</span>
                        <span className="text-ink-soft text-[11px] text-right">{f.label.split('—')[1] || f.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="max-h-96 overflow-y-auto space-y-1.5 font-mono pr-1">
                    {PDVSA_ACES_C.map(a => (
                      <div key={a.code} className="p-2 bg-surface-2 rounded-lg flex justify-between border border-line">
                        <span className="font-bold text-brand-500">{a.code}</span>
                        <span className="text-ink-soft text-[11px] text-right">{a.label.split('—')[1] || a.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="max-h-96 overflow-y-auto space-y-1.5 font-mono pr-1">
                    {PDVSA_AREAS_GEOGRAFICAS_C.map(g => (
                      <div key={g.code} className="p-2 bg-surface-2 rounded-lg flex justify-between border border-line">
                        <span className="font-bold text-emerald-500 dark:text-emerald-400">{g.code}</span>
                        <span className="text-ink-soft text-[11px] text-right">{g.label.split('—')[1] || g.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="bg-surface rounded-2xl border border-line p-6 shadow-2xs space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-xs font-mono">
                {PDVSA_INSTALACIONES_D.map(inst => (
                  <div key={inst.code} className="p-3 bg-surface-2 rounded-xl border border-line flex items-center justify-between">
                    <span className="text-base font-black text-brand-500 w-10">{inst.code}</span>
                    <span className="text-ink font-bold text-right text-[11px]">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-xs">
                ...
                <div className="p-4 bg-surface-2 rounded-xl border border-line space-y-3">
                  ...
                  <div className="space-y-1.5 font-mono">
                    {PDVSA_DISCIPLINAS_J.map(d => (
                      <div key={d.code} className="p-2 bg-surface rounded-lg flex justify-between items-center border border-line">
                        <span className="font-black text-brand-500 text-sm">{d.code}</span>
                        <span className="font-bold text-ink">{d.label.split('—')[1] || d.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Products LL */}
                <div className="p-4 bg-surface-2 rounded-xl border border-line space-y-3">
                  <h3 className="font-bold text-emerald-500 dark:text-emerald-400 uppercase text-xs border-b border-line pb-2">
                    Productos de Ingeniería Estándar (LL)
                  </h3>
                  <div className="max-h-96 overflow-y-auto space-y-1.5 font-mono pr-1">
                    {PDVSA_PRODUCTOS_LL.map(pr => (
                      <div key={pr.code} className="p-2 bg-surface rounded-lg flex justify-between items-center border border-line">
                        <span className="font-bold text-emerald-500 dark:text-emerald-400">{pr.code}</span>
                        <span className="text-ink-soft text-[11px] text-right">{pr.label.split('—')[1] || pr.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-surface rounded-2xl border border-line p-6 shadow-2xs space-y-5">
            <div className="border-b border-line pb-3">
              <h3 className="font-bold text-base text-ink flex items-center gap-2">
                <Flame size={20} className="text-brand-500 shrink-0" />
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <label className="block font-bold text-ink-soft mb-1">NPS (Pulgadas)</label>
                <select
                  value={weldNpsInches}
                  onChange={(e) => setWeldNpsInches(Number(e.target.value))}
                  className="w-full px-3 py-2 bg-surface-2 border border-line rounded-xl font-bold text-ink"
                >
              ...
              <div>
                <label className="block font-bold text-ink-soft mb-1">Espesor Pared (mm)</label>
                <input
                  type="number"
                  step="0.1"
              ...
              <div className="col-span-2">
                <label className="block font-bold text-ink-soft mb-1 flex items-center justify-between">
                  <span>Tipo de Electrodo / Varilla AWS</span>
                  <span className="text-[10px] text-brand-500 font-mono">Rendimiento Técnico: {currentElectrode.yieldPct}%</span>
                </label>
                <select
                ...
                <p className="text-[10px] text-ink-soft mt-1 italic">{currentElectrode.note}</p>
              </div>

              <div>
                <label className="block font-bold text-ink-soft mb-1">Cantidad de Juntas</label>
                <input
                  type="number"
                  min={1}
              ...
              <div>
                <label className="block font-bold text-ink-soft mb-1">Ajuste Rendimiento (%)</label>
                <input
                  type="number"
                  ...
                  className="w-full px-3 py-2 bg-surface-2 border border-line rounded-xl font-bold text-ink placeholder:text-ink-soft"
                />
              </div>
            </div>

            <div className="p-4 bg-surface-2 rounded-xl border border-line font-mono text-xs space-y-2 text-ink">
              <div className="flex justify-between">
                <span className="text-ink-soft">Metal Neta Depositado:</span>
                <span className="font-bold">{weldResults.netKg.toFixed(2)} kg</span>
              </div>
              <div className="flex justify-between text-ink-soft">
                <span>Rendimiento Técnico de Deposición:</span>
                <span className="font-bold text-brand-500">{weldResults.effPct.toFixed(0)}%</span>
              </div>
              ...
              <div className="flex justify-between text-emerald-600 dark:text-emerald-400 font-bold border-t border-line pt-1.5 text-sm">
          ...
          <div className="bg-surface rounded-2xl border border-line p-6 shadow-2xs space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div>
                <label className="block font-bold text-ink-soft mb-1">Tipo de Fluido</label>
                <select
                  value={spanFluid}
            ...
            <div className="p-4 bg-surface-2 rounded-xl border border-line font-mono text-xs space-y-2 text-ink">
              <div className="flex justify-between items-center">
                <span className="text-ink-soft">Luz Libre Máxima Recomendada:</span>
                <span className="text-xl font-black text-emerald-600 dark:text-emerald-400">{spanResultM.toFixed(2)} metros</span>
              </div>
              <p className="text-[10px] text-ink-soft border-t border-line pt-1.5">
